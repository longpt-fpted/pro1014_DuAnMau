<?php
    ...
    if($category == "All" and $sort == "Default"){
        ...
        $cate = $cateDAO->getCategoryByName($category);
        $id = $cate->getID();
        $sort = $productDAO->sortProductPriceDecrease($id,$min,$max);
    } else if ($category != "All" and $sort == "FromA-Z"){
        $cate = $cateDAO->getCategoryByName($category);
        $id = $cate->getID();
        $sort = $productDAO->sortProductFromAToZ($id,$min,$max);
    } else if ($category != "All" and $sort == "FromZ-A"){
        $cate = $cateDAO->getCategoryByName($category);
        $id = $cate->getID();
        $sort = $productDAO->sortProductFromZToA($id,$min,$max);
    } else if ($category != "All" and $sort == "HighSale"){
        $cate = $cateDAO->getCategoryByName($category);
        $id = $cate->getID();
        $sort = $productDAO->sortProductHighSale($id,$min,$max);
    } else if ($category != "All" and $sort == "HighSale"){
        $cate = $cateDAO->getCategoryByName($category);
        $id = $cate->getID();
        $sort = $productDAO->sortProductHighSale($id,$min,$max);
