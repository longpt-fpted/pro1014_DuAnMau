...
class NotifyDAO {
    ...
    public function getAllNotifiesByUserID($userID) {
        if($this->database->connect_error) {
            ...
            $query = $this->database->prepare("SELECT *, DATE_FORMAT(`notify`.`date`, '%d/%l/%Y') AS `fdate` FROM `Notify` WHERE `Notify`.`user_id` = ?");
            ...
            if($query->execute()) {
                ...
                if($result->num_rows > 0) {
                    ...
                    while($row = $result->fetch_assoc()) {
                        $notify = new Notify($row['id'], $row['user_id'], $row['type'], $row['type_id'], $row['title'], $row['fdate']);
                        $notifies[] = $notify;
                    }
                    return $notifies;
                } else return false;
